package ua.com.proteus.specification;
...
public class GraphicSpecification implements Specification<Graphic> {
	...
	public GraphicSpecification(GraphicFilter filter) {
		...
		if(REG.matcher(filter.getMaxFrequency()).matches()){
			filter.setMaxFrequencyValue(new Integer(filter.getMaxFrequency()));
		}
		if(REG.matcher(filter.getMinFrequency()).matches()){
			filter.setMinFrequencyValue(new Integer(filter.getMinFrequency()));
		}
		if(REG.matcher(filter.getMaxData_bus()).matches()){
			filter.setMaxData_busValue(new Integer(filter.getMaxData_bus()));
		}
		if(REG.matcher(filter.getMinData_bus()).matches()){
			filter.setMinData_busValue(new Integer(filter.getMinData_bus()));
		}
		if(REG.matcher(filter.getMaxMemory_value()).matches()){
			filter.setMaxMemoryValue(new Integer(filter.getMaxMemory_value()));
		}
		if(REG.matcher(filter.getMinMemory_value()).matches()){
			filter.setMinMemoryValue(new Integer(filter.getMinMemory_value()));
		}
	}
